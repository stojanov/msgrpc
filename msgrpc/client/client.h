...
namespace msgrpc::client
{
    ...
    public:
                                                                          ...
                                                                          core::listener_callback<rtn_type> listener = nullptr)
        {
            ...
            auto call = std::shared_ptr<core::call_handler<param_type, rtn_type>>(
                    new core::call_handler<param_type, rtn_type>(name, std::move(server_call), std::move(listener)));

            // we override if there is already a call handler attach, shouldn't change much
            m_calls[name] = call;
    ...
    private:
        ...
        {
            return m_id_counter.fetch_add(1);
        }

        template<typename param_type, typename rtn_type>
        static call_resolver_expected<param_type, rtn_type> call_function(
                std::weak_ptr<core::call_handler<param_type, rtn_type>> call_handler_ptr,
                param_type& parameter)
        {
            ...
            auto possibleId = _call_handler->send_call(parameter);

            if (possibleId)
            {
                auto call_resolver = std::make_shared<core::call_resolver<param_type, rtn_type>>(possibleId.value(), _call_handler);
                return call_resolver;
            }
            
            return possibleId.error();
        }
